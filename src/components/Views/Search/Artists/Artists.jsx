...
export default function Artists()
{
	...
	const { title, artistFollowers } = getColDef();
	const { table, error } = useInfiniteTable(`search/?q=${params.get("q")}&type=artist&limit=50`,
		[ title, artistFollowers ],
		...
		(data) => {
			...
			const tableData = data.pages.reduce((acc, page) => {
				acc.push(... page.artists.items);
				return (acc);
			}, []);
			return ([tableData, data.pages[0].artists.total]);
		},
		(lastPage) => lastPage.artists.offset < 150 ? lastPage.artists.next : null
	);
